...
const NewlyMarriedKookminApi = ({ onSaveData }) => {
    ...
    const rankSuccess = async () => {
        if (form?.newlyMarriedKookminRes === '1순위') {
            history.push({
                pathname: '/firstRank',
                state: {
                    form,
        ...
        } else if (form?.newlyMarriedKookminRes === '2순위') {
            history.push({
                pathname: '/secondRank',
                state: {
                    form,
        ...
        <>
            ...
            <form className="specialSupply_form" onSubmit={handleSubmit}>
                ...
                <div className="rankRes">
                    ...
                    (data?.meetMonthlyAverageIncomePriority === true ||
                        data?.meetMonthlyAverageIncomeGeneral === true) &&
                    ...
                    {/* 탈락 */}
                    {data?.accountTf === false ||
                    (data?.americanAge < 20 && form.supportYn === 'n') ||
                    data?.meetHomelessHouseholdMembersTf === false ||
                    data?.meetMarriagePeriodIn7yearsTf === false ||
                    (data?.meetMonthlyAverageIncomePriority === false &&
                        data?.meetMonthlyAverageIncomeGeneral === false) ||
